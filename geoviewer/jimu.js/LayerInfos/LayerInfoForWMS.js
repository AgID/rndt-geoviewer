...
LayerInfo, LayerInfoForDefaultWMS, LayerInfoFactory, FeatureLayer, esriLang) {
  ...
  return declare(LayerInfo, {
    ...
    obtainNewSubLayers: function(layerInfos, parentLayerInfo) {
	  var that = this;	
    var newSubLayerInfos = [];
	  if (!layerInfos) layerInfos = this.layerObject.layerInfos;
      array.forEach(layerInfos, function(layerInfo, index){
        var subLayerInfo;
        subLayerInfo = LayerInfoFactory.getInstance().create({
          ...
          selfType: 'wms',
          parentLayerInfo: parentLayerInfo || this
          //,newSubLayers : that.obtainNewSubLayers(layerInfo.subLayers, layerInfo, newSubLayerInfos);
        });
        if(layerInfo.subLayers && layerInfo.subLayers.length>0) {
         // newSubLayerInfos.push(subLayerInfo);	subLayerInfo.init();
          newSubLayerInfos = newSubLayerInfos.concat(that.obtainNewSubLayers(layerInfo.subLayers, layerInfo));
        }else{	
