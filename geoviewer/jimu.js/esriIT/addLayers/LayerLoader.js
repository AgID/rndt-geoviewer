      ...
      */
      ...
      _newWMSLayer: function() {
        ...
        dfd.then(function(layer) {
          self._setWMSlayerInfosParentLayer(layer.layerInfos,null,null);
          self._setWMSVisibleLayers(layer);
          layer.on( "update-end", lang.hitch(this, self._checkLayersNull ,layer)); 
        });
        return dfd;
      },
      
    _setWMSlayerInfosParentLayer: function(layerInfos, parentLayer){
      var that = this;
      array.forEach(layerInfos,function (layerInfo, ii){
        if( !layerInfo.hasOwnProperty("parentLayer") ){
          layerInfo.parentLayer = parentLayer;
          if ( layerInfo.subLayers.length > 0 ) {
            that._setWMSlayerInfosParentLayer(layerInfo.subLayers, layerInfo);
          }
         } 
      });
    },
      ...
      _setWMSVisibleLayers: function(layer) {
        ...
        if (layer) {
          array.some(layer.layerInfos, function(lyrInfo) {
            ...
            if (typeof lyrInfo.name === "string" && lyrInfo.name.length > 0 && lyrInfo.subLayers.length==0) {
          ...
          if (lyrNames.length > 0 && lyrNames.length <= maxLayers) {
      ...
      _checkLayersNull: function(lyr){      
        var ary = lang.clone(lyr.visibleLayers);
        var visibleLayers = ary.filter(function(item){
          return item.toString().trim().length == 0 ? false : true;
        });    
        if(ary.length != visibleLayers.length){
